...
def calc_nf2ff_error(filename_nearfield, filename_farfield, theta_lim, theta_steps, phi_lim, phi_steps,
                     separation, frequency_block, xy_error, z_error, linestyle='-',
                     plots=False,
                     eplots = False,
                     e_ideal_plots = False,
                     hplots = False,
    ...
    if(eplots):
        ...
        plt.plot(np.rad2deg(theta_grid_ff[0]), e_cut_nf_n, linestyle)
        plt.xlim(-90, 90)
        plt.ylim(-60, 0)
        plt.grid(True)
